 ...
 * 用 value 获取枚举值
 */
export function enumValueOf(e: any, value: any) {
  for (const key in e) {
    const val = e[key];
...
export function enumKeyOf(e: any, value: any) {
  for (const key in e) {
    const val = e[key];
    if (key === value) {
      return val;
...
export const THREAD_POOL_METRICS_TYPE = {
  TERMINAL: {
    value: 10,
    label: "远程终端线程池",
    schedulerPoolKey: "TERMINAL_SCHEDULER",
  },
  TERMINAL_WATCHER: {
    value: 15,
    label: "终端监视线程池",
    schedulerPoolKey: "TERMINAL_WATCHER_SCHEDULER",
  },
  EXEC: {
    value: 20,
    label: "批量执行线程池",
    schedulerPoolKey: "EXEC_SCHEDULER",
  },
  TAIL: {
    value: 30,
    label: "文件追踪线程池",
    schedulerPoolKey: "TAIL_SCHEDULER",
  },
  SFTP_TRANSFER_RATE: {
    value: 40,
    label: "传输进度线程池",
    schedulerPoolKey: "SFTP_TRANSFER_RATE_SCHEDULER",
  },
  SFTP_UPLOAD: {
    value: 50,
    label: "文件上传线程池",
    schedulerPoolKey: "SFTP_UPLOAD_SCHEDULER",
  },
  SFTP_DOWNLOAD: {
    value: 60,
    label: "文件下载线程池",
    schedulerPoolKey: "SFTP_DOWNLOAD_SCHEDULER",
  },
  SFTP_PACKAGE: {
    value: 70,
    label: "文件打包线程池",
    schedulerPoolKey: "SFTP_PACKAGE_SCHEDULER",
  },
  APP_BUILD: {
    value: 80,
    label: "应用构建线程池",
    schedulerPoolKey: "APP_BUILD_SCHEDULER",
  },
  RELEASE_MAIN: {
    value: 90,
    label: "应用发布主线程池",
    schedulerPoolKey: "RELEASE_MAIN_SCHEDULER",
  },
  RELEASE_MACHINE: {
    value: 100,
    label: "应用发布机器线程池",
    schedulerPoolKey: "RELEASE_MACHINE_SCHEDULER",
  },
  SCHEDULER_TASK_MAIN: {
    value: 110,
    label: "调度任务主线程池",
    schedulerPoolKey: "SCHEDULER_TASK_MAIN_SCHEDULER",
  },
  SCHEDULER_TASK_MACHINE: {
    value: 120,
    label: "调度任务机器线程池",
    schedulerPoolKey: "SCHEDULER_TASK_MACHINE_SCHEDULER",
  },
  PIPELINE: {
    value: 130,
    label: "应用流水线线程池",
    schedulerPoolKey: "PIPELINE_SCHEDULER",
  },
};
...
export const MESSAGE_TYPE = {
  EXEC_SUCCESS: {
    ...
    label: "命令执行完成",
    notify: "success",
    redirect: (row: any) => `/batch/exec/list?id=${row.relId || ""}`,
  },
  EXEC_FAILURE: {
    classify: 10,
    value: 1020,
  ...
  REPOSITORY_INIT_SUCCESS: {
    ...
    label: "版本仓库初始化成功",
    notify: "success",
    redirect: (row: any) => `/app/repo?id=${row.relId || ""}`,
  },
  ...
  BUILD_SUCCESS: {
    ...
    label: "构建执行成功",
    notify: "success",
    redirect: (row: any) => `/app/build/list?id=${row.relId || ""}`,
  },
  BUILD_FAILURE: {
    classify: 10,
    value: 1060,
  ...
  RELEASE_AUDIT_RESOLVE: {
    ...
    label: "发布审批通过",
    notify: "success",
    redirect: (row: any) => `/app/release/list?id=${row.relId || ""}`,
  },
  ...
  PIPELINE_AUDIT_RESOLVE: {
    ...
    label: "应用流水线审批通过",
    notify: "success",
    redirect: (row: any) => `/app/pipeline/task?id=${row.relId || ""}`,
  },
  ...
  MACHINE_AGENT_INSTALL_SUCCESS: {
    ...
    notify: "success",
    redirect: (row: any) =>
      `/machine/monitor/list?machineId=${row.relId || ""}`,
  },
  ...
  MACHINE_ALARM: {
    ...
    notify: "error",
    duration: null,
    redirect: (row: any) => `/machine/monitor/metrics/${row.relId || ""}`,
  },
};
