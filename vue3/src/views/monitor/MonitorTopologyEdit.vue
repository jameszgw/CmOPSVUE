    ...
    >
      <div v-loading="drawer.loading" class="drawer-body">
        ...
        <div v-if="drawer.data?.source" class="drawer-row">
          ...
          <el-tag :type="['agent','ssh','snmp','winrm','redis'].includes(drawer.data.source) ? 'success' : 'info'" size="small">
            {{ {agent:'真实采集·Agent',ssh:'真实采集·SSH',snmp:'真实采集·SNMP',winrm:'真实采集·WinRM',redis:'真实采集·Redis'}[drawer.data.source] || '模拟数据' }}
          </el-tag>
        </div>
