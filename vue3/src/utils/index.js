// 由原 React 工程 src/utils/index.tsx 转换而来（仅移除 TypeScript 类型标注，内容不变）。
// 说明：原文件虽为 .tsx，但不包含任何 JSX，全部为纯数据枚举常量与工具函数，
// 其中的 label/color/status 等字段由页面自行渲染（如 el-tag 的颜色与文案）。
...
export function enumKeyOf(e, value) {
  for (const key in e) {
    const val = e[key];
    if (key === value) {
      return val;
